...
module "security_group_EC2" {
  ...
  type         = "EC2"
  vpc_id       = local.vpc_id
  ingress_cidr = ["${local.my_pub_ip}/32", var.vpc_cidr]
  egress_cidr  = [var.vpc_cidr, "${local.my_pub_ip}/32"]
  prefix       = local.prefix
  default_tags = local.default_tags
}
...
module "security_group_Bastion" {
  ...
  type         = "Bastion"
  vpc_id       = local.vpc_id
  ingress_cidr = [var.vpc_cidr, "${local.cloud_pub_ip}/32", "${local.cloud_pri_ip}/32"]
